...
The Stage 4 prompt requires exact section headers (`CONSENSUS`, `CONFIDENCE`, `DECISION`, `RECOMMENDATION`, `NEXT STEPS`, `TRIGGER CONDITIONS`, `POINTS OF AGREEMENT`, `POINTS OF DISAGREEMENT`). Before parsing, `_normalize_chair_headers()` strips markdown decoration (`### CONSENSUS`, `**DECISION:**` → `CONSENSUS:`, `DECISION:`) — GLM drifts into markdown headers whenever a prompt doesn't restate the exact format, which is exactly what broke follow-up turns on 2026-07-19 (empty card + default NO CONSENSUS badge while the raw text said "unanimously agrees"). Both the initial AND follow-up chairman prompts now restate the full format verbatim, demand PLAIN TEXT headers, and require TRIGGER CONDITIONS on a split.

`DECISION` normalizes to `APPROVE` / `APPROVE WITH CONDITIONS` / `REJECT` / `NO CONSENSUS` — and `NO CONSENSUS` is a board VERDICT, never a fallback. Two honest non-verdict states exist instead: `UNPARSED` (nothing extracted; UI badges "Unstructured — see full output" and auto-expands the raw as markdown) and `UNCLASSIFIED` (sections parsed but the Chair decided in prose; the prose lands in `decision_note`, rendered as a "Decision Detail" section, badge "Decided — see detail"). Never guess a verdict the Chair didn't state.

Storage (`board_storage.py`, `storage.py`) opens all files with explicit `encoding="utf-8"` — Windows default cp1252 corrupts/crashes on any non-ASCII in stored turns (bit us 2026-07-19 after a re-parse migration wrote real UTF-8).
